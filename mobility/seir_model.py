...
class SIR:
    def __init__(self, num_cbg, num_poi, graph_weights, 
        ...
        init_exposed = self._sample_from_binomial(self.num_population, self.p_0)
        self.num_susceptible[0, :] = self.num_population - init_exposed
        self.num_infected[0, :] = init_exposed
    ...
    def run_one_iteration(self, t, mobility_weights):
        ...
        num_S_to_I = self._sample_from_poisson(lambda_poi_sum*S/N) \
                        + self._sample_from_binomial(S, lam_cbg)
        num_I_to_R = self._sample_from_binomial(I, 1/self.delta_I)

        # In case they become < 0
        num_S_to_I = np.minimum(num_S_to_I, S)
        num_I_to_R = np.minimum(num_I_to_R, I + num_S_to_I)

        self.num_susceptible[t+1, :] = S - num_S_to_I
        self.num_infected[t+1, :] = I + num_S_to_I - num_I_to_R
        self.num_recovered[t+1, :] = R + num_I_to_R
...
class SIS:
    ...
    def run_one_iteration(self, t, mobility_weights):
        ...
        num_S_to_I = self._sample_from_poisson(lambda_poi_sum*S/N) \
                        + self._sample_from_binomial(S, lam_cbg)
        num_I_to_S = self._sample_from_binomial(I, 1/self.delta_I)

        # In case they become < 0
        num_S_to_I = np.minimum(num_S_to_I, S)
        num_I_to_S = np.minimum(num_I_to_S, I + num_S_to_I)

        self.num_susceptible[t+1, :] = S - num_S_to_I + num_I_to_S
        self.num_infected[t+1, :] = I + num_S_to_I - num_I_to_S
